...
class Design_53_53_spawnTrees extends SceneScript
{
	...
	{
		if((Engine.engine.getGameAttribute("virtualX") < 50))
		{
			createRecycledActor(getActorType(70), randomInt(Math.floor(-50), Math.floor(-20)), -10, Script.FRONT);
	...
	{
		if((450 < (Engine.engine.getGameAttribute("virtualX") + getScreenWidth())))
		{
			createRecycledActor(getActorType(70), randomInt(Math.floor(230), Math.floor(270)), -10, Script.FRONT);
			getLastCreatedActor().makeAlwaysSimulate();
			getLastCreatedActor().sendToBack();
			_customEvent_treeType();
		}
